        ...
        let calls = response
            ...
            .map(|tc| ParsedToolCall {
                ...
                arguments: serde_json::from_str(&tc.arguments).unwrap_or_else(|e| {
                    ::zeroclaw_log::record!(WARN, ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note).with_outcome(::zeroclaw_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"tool": tc.name, "error": e.to_string()})), "Failed to parse native tool call arguments as JSON; defaulting to empty object");
